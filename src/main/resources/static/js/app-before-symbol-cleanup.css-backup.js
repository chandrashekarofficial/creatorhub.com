...
const FAVORITES_KEY = "creatorhub_favorite_channels";
...
function toggleFavorite(channel) {
    ...
    if (existingIndex >= 0) {
        ...
        if (favorites.length >= MAX_FAVORITES) {
            $("error").textContent =
                "You can save up to 5 favorite channels. Remove one first.";
            return;
        }
...
function renderFavorites() {
    ...
    grid.innerHTML = favorites.map(channel => `
        <button
            class="favorite-card"
            type="button"
            data-channel-id="${esc(channel.channelId)}"
        >
            <img
    ...
    grid.querySelectorAll(".favorite-card").forEach(card => {
        card.addEventListener("click", () => {
            ...
            loadFavoriteChannel(channelId);
        });
    });
}

async function loadFavoriteChannel(channelId) {
    const button = $("go");
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(
                data.message ||
                data.error ||
                "Unable to load favorite channel."
            );
        }
